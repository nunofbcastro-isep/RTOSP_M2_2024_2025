...
{
    printf("TASK:%-3d  %12llu  %12llu  %12llu  %12llu  %8llu  %6u\n",
           task->id,
           task->C1,
           task->C2,
           task->C2,  // Verifica se é mesmo suposto repetir C2
           task->T,
           task->O,
           task->njobs);
}
...
{
    printf("TASK        %-12s  %-12s  %-12s %-12s  %-8s  %-6s\n",
           "C1", "C2", "C3", "T", "O", "njobs");
}
...
///////////////////////////////////////////////////////////////
	...
	for(i=0;i<ntasks;i++){
		...
		sprintf(arg[1],"%llu",tasks[i].C1);
		sprintf(arg[2],"%llu",tasks[i].C2);
		sprintf(arg[3],"%llu",tasks[i].C3);
		sprintf(arg[4],"%llu",tasks[i].T);
		sprintf(arg[5],"%llu",tasks[i].O);
		sprintf(arg[6],"%llu",time0);
		sprintf(arg[7],"%d",tasks[i].njobs);
